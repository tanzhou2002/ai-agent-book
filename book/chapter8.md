...
这条 Skill 还可以成为第七章后训练的数据规范：从带作用域标签的文档蓝图合成正例、保护区反例和歧义样本，经过验证器过滤后做 SFT。要把可信度从机制演示提升到生产回归级别，合成数据不能只增加条数，还要按文章体裁、作用域类型和编程语言分层，并对每层做人工抽查；机器门禁检查未解析变量、代码围栏、非注释代码行漂移和保护区保持，人工记录则保存抽查依据。训练后不应删除 Skill；它仍然是可读规范、合成数据生成器、模型回归集和规则变更时的重训依据。模型参数内化的是跨任务识别作用域和采取正确动作的习惯，而不是取代规范本身。

特殊字符串的 Bad Case 则走另一条路径。若 `old_string` 在模型直接复述探针中已经发生 byte-level 漂移，可以用第七章的 copy-focused SFT 和 copy replay 恢复逐 token 保真度；但必须把 tokenizer 作为独立变量：在 Qwen、Llama/Mistral 等不同开源 tokenizer 上测量 encode→decode 的无损 round-trip、特殊字符覆盖和 token 长度分布。若模型输出正确而 tokenizer、序列化或工具接口改变了字符串，应更新 Harness 或工具，而不是创建一个 Skill 掩盖系统错误。这两个案例应分别报告 Skill 遵循率、保护区误伤率、byte-exact 复制率和 tokenizer round-trip，不能合成一个“文本质量”总分。
