...
> **التجربة 7-17 ★★: من حالات «الإكمال المبكر» إلى DPO وGRPO**
>
> تصل التجربة إخفاقات الفصل السادس بمسارين للتدريب. يحول `build_preference_data.py` 24 حالة إلى أزواج تفضيل لبادئة المسار: إعلان الإكمال الأصلي rejected، والفعل الذي يتحقق أولًا chosen. يوفر `train_dpo.py` مسار LoRA-DPO لـ Qwen2.5-7B-Instruct، ويستخدم مسار GRPO الاختياري اختبارات قبول مخفية.
>
> ارتفعت القرارات الصحيحة في مجموعة الحدود من 3/12 (25.0%) إلى 11/12 (91.7%)، وبقيت مجموعة المهام المكتملة 8/8 (100%). أصبح التوليد الحر حذرًا أكثر من اللازم، لذا فالمقارنة الثابتة هي النتيجة الرئيسية وليست ادعاءً بتحسن النجاح العام في التشغيل. التفاصيل في [`premature-completion-dpo`](../chapter7/premature-completion-dpo/).

> **التجربة 7-18 ★★: تدريب SFT للاقتباس المنحني الحساس للنطاق**
>
> حُوّلت حالة الاقتباس المستقيم الصيني إلى Skill بقواعد موجبة وسالبة للنطاق. يمكن تحويل اقتباس النثر والتعليق الصيني إلى اقتباس منحني، بينما تبقى اقتباسات الإنجليزية وسلاسل الكود وJSON والمسارات والمعرّفات وبنية التنفيذ كما هي byte بbyte. غطت البيانات 10 أنواع مقالات و9 لغات برمجة، بمقادير 1024/256/256 للتدريب والحجز والحدود.
>
> حقق Qwen3-8B مع LoRA وbf16 على RTX PRO 6000 دقة exact قدرها 96.9% في الحجز و97.7% في الحدود، مع حفظ المناطق المحمية بنسبة 100%. بلغت فئات Python وJavaScript وJava وGo وRust وSQL وShell وYAML وMarkdown نسبة 100%، بينما بقيت JSON عند 68.8% وتحتاج مسارًا بنيويًا مستقلًا.

> **التجربة 7-19 ★★: SFT للنسخ الدقيق للسلاسل الخاصة**
>
> يُشخّص فشل `old_string` طبقيًا: بايتات الملف، إرجاع الأداة، تسلسل Harness، خرج النموذج، tokenizer، ثم المطابقة. لا تدخل بيانات SFT إلا الانحراف الناتج عن النموذج. تضم المجموعة الموسعة 1024/256/256 عينة، وسياقات لغات متعددة، وhard negatives، ومسافات وescape وUnicode ومحارف بعرض صفري ووسائط JSON.
>
> رفع LoRA bf16 لـ Qwen3-8B الدقة byte-exact من 37.5% إلى 78.9% في الحجز وإلى 80.1% في الحدود. أظهر تدقيق 512 probe أن round-trip في Qwen3/Qwen2.5 هو 80.1% مقابل 100% في Mistral؛ لذلك يجب فصل أخطاء tokenizer/Harness عن قدرة النسخ لدى النموذج.
